...
test_rend_cache_lookup_v2_desc_as_dir(void *data)
{
  ...
  // Test existing descriptor  shuai:in this place,at first save,then fetch
  generate_desc(RECENT_TIME, &desc_holder, &service_id, 3);
...
{	
 ...
 //test by yinshuai 
  printf("\n****************** test by yinshuai ***************\n");
  generate_desc(RECENT_TIME+1, &desc_holder, &service_id, 3);
  ret = rend_cache_store_v2_desc_as_dir(desc_holder->desc_str);
  tt_int_op(ret, OP_EQ, 0); 
  printf("\n*****************test by yinshuai  end ***************\n");
