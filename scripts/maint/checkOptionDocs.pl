...
open(F, "/torcode/tor-0.2.9.8/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/torcode/tor-0.2.9.8/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/torcode/tor-0.2.9.8/doc/tor.1.txt") or die;
while (<F>) {
